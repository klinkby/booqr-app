...
- **Build**: Two-stage Docker builds; lighttpd web server; tree-shaken, code-split chunks (no manual vendor split)
  targeting ES2022.
...
- **Locale selection**: The configured strategy is `localStorage`, browser `preferredLanguage`, then `baseLocale`.
  ...
  former `locale-override` key. Do not introduce another locale store or a separate persistence key.
- **Reload on switch**: Language changes deliberately reload the SPA. This is Paraglide's supported approach for this
  static, client-rendered application, ensuring every translated module is initialized for the new locale. Do not call
  `setLocale(..., { reload: false })` for the footer picker.
- **Root layout**: `src/routes/+layout.svelte` owns document integration. Its component-scoped `$effect` applies
  `getLocale()` and `getTextDirection()` to `<html lang>` and `dir`; `$effect` must never be used in a module-level
  singleton such as `locale.svelte.js`.
